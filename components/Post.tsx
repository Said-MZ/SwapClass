...
const Post: React.FC<PostProps> = async ({ post, userEmail }) => {
  ...
  return (
    <Link
      ...
      key={post.id}
      className="relative flex flex-col gap-2  backdrop-blur-[1px] p-4 border-2 border-neutral-700 rounded-md sm:hover:brightness-125 sm:hover:-translate-y-1 sm:hover:border-neutral-600 transition-all duration-200"
    >
      {isUser && (
